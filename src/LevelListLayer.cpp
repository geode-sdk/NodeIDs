...
$register_ids(LevelListLayer) {
	...
	// the below code (before any layout manipulation) appears to work fine
	// for windows, macos, and android (using latest bindings as of may 23 2024).
	// however, as HJFod plans to add ColumnLayout stuff (see comments after line 130)
	// everything else in here has been commented out to avoid API breaks in the future.
	// the $register_ids macro is still here in order to assign LevelListLayer itself
	// a proper node ID without forcing developers to call getChildOfType() to detect
	// this particular layer (or even worse, calling this->setID() manually).
	// contrarary to past personal misconceptions, the source code was not
	// commented out because of macos-specific crashes from this specific file..
	// such crashes happened from building Node IDs against outdated bindings.
	// for an artifact that assigns the other node IDs below this comment,
	// use this actions run:
	// https://github.com/RayDeeUx/NodeIDs/actions/runs/9210273062
	// (link valid for ~90 days [probably?] from may 23 2024)
	// - erymanthus
